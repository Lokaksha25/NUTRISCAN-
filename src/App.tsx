...
function LandingPage({ onGetStarted }: { onGetStarted: () => void }) {
  return (
    <div className="min-h-screen bg-background flex flex-col items-center justify-center p-6 relative overflow-hidden">
        {/* Background blobs or gradient */}
        <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-primary/10 rounded-full blur-3xl pointer-events-none"></div>
        <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-secondary rounded-full blur-3xl pointer-events-none"></div>

        <div className="max-w-3xl text-center z-10 w-full">
            <div className="inline-flex items-center gap-2 px-3 py-1 bg-secondary text-foreground rounded-full text-xs font-bold tracking-widest mb-8 border border-border/50">
            ...
            <h1 className="text-5xl md:text-7xl font-black tracking-tighter mb-6 text-foreground drop-shadow-sm leading-tight">
                Decode your food. <br/>
                <span className="text-muted-foreground">Protect your health.</span>
            </h1>
            ...
            <Button size="lg" onClick={onGetStarted} className="rounded-full px-8 h-14 text-base font-bold shadow-lg shadow-primary/25 hover:shadow-xl hover:-translate-y-0.5 transition-all w-full sm:w-auto">
                Get Started
...
export default function App() {
  ...
  const parsedResult = useMemo<ReportData | null>(() => {
    ...
    try {
      // Find JSON array or object if wrapped in markdown
    ...
    <div className="min-h-screen bg-background font-sans text-foreground transition-colors duration-300">
      ...
      <header className="bg-white/70 dark:bg-black/70 backdrop-blur-xl border-b border-border/50 px-6 py-4 flex items-center justify-between sticky top-0 z-50">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg font-bold tracking-tight uppercase tracking-widest opacity-80">NutriScan</h1>
        </div>
        ...
        <div className="lg:col-span-8 flex flex-col h-full relative">
            ...
            {parsedResult ? (
               <div className="bg-transparent flex-1 h-full rounded-3xl overflow-hidden flex flex-col relative w-full h-[calc(100vh-120px)]">
                        ...
                        <div className="col-span-full bg-card border border-border/80 rounded-[2.5rem] p-8 shadow-sm backdrop-blur-2xl flex flex-col md:flex-row gap-8 items-center md:items-stretch overflow-hidden relative">
                           ...
                           <div className="flex-shrink-0 flex flex-col justify-center items-center md:items-start text-center md:text-left min-w-[200px] z-10">
                              ...
                              <div className="flex items-baseline gap-1">
                                <span className="text-7xl font-black tracking-tighter text-foreground">{parsedResult.healthScore?.total || 0}</span>
                ...
                // Fallback rendering incase JSON string fails parsing but still is there
